...
function wrapSelection(tag) {
    ...
    if (selection) {
        ...
        let from = editor1.getCursor("from");
        let to = editor1.getCursor("to");
        let wrappedText = `<${tag}>${selection}</${tag}>`;
        editor1.replaceRange(wrappedText, from, to);
        let insertStart = editor1.indexFromPos(from);
        ...
        if (match) {
            ...
            let newToPos = editor1.posFromIndex(insertStart + closingTagEndOffset);
            editor1.setSelection(from, newToPos);
        } else console.warn("Could not find closing tag position correctly.");
        editor1.focus();
    }
}
...
//# sourceMappingURL=survey3.f655706d.js.map
