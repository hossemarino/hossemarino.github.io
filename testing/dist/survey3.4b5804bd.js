...
function generateIHUTXML() {
  ...
  <samplesource list="3">
    ...
    let ihutCSS = ``;
    if (xmlData.surveyType !== "Screener" && OTS) ihutCSS = IHUT_OTS_CSS;
    let consentQ = '';
    ...
    let cltNote = ``;
    if (xmlData.surveyType === "CLT") cltNote = CLTNOTE;
    let ots_api = ``;
    let restOfSurvey_1 = ``;
    let restOfSurvey_2 = ``;
    let restOfSurvey_3 = ``;
...
`;
        ...
        if (xmlData.surveyType === "Questionnaire/Diary" && OTS) ots_api = OTS_API;
        ...
        if (xmlData.verity === "Yes" && xmlData.setupType !== "QualBoard - user creation API" && xmlData.contactQuestion === "Yes") {
            ...
            if (!restOfSurvey_2.includes(VERITY_API)) restOfSurvey_3 += VERITY_API;
        }
    }
    const xml = `
...
//# sourceMappingURL=survey3.4b5804bd.js.map
