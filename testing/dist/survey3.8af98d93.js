...
function makeStateSelect({ addRecode = false } = {}) {
    try {
        ...
        const choicesXml = US_STATES.map(([code, name])=>`  <choice label="${code}">${name}</choice>`).join("\n");
...
function makeStateCheckbox() {
    try {
        ...
        let title = match[3].trim().replace(/\n{2,}/g, "\n");
        if (/^\d/.test(label)) label = "Q" + label;
        const rows = US_STATES.map(([code, name])=>`  <row label="${code}">${name}</row>`).join("\n");
...
//# sourceMappingURL=survey3.8af98d93.js.map
